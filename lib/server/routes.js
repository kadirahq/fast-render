...
FastRender.route = function route(path, callback) {
  ...
  function onRoute(params, req, res, next) {
    var afterProcessed = setQueryDataCallback(res, next);
    FastRender._processRoutes(params, req, callback, afterProcessed);
  }
};
...
function setQueryDataCallback(res, next) {
  return function(queryData) {
    if(!res.queryData) {
      ...
      _.extend(res.queryData.subscriptions, queryData.subscriptions);
      _.each(queryData.collectionData, function(data, pubName) {
        var existingData = res.queryData.collectionData[pubName]
        if(existingData) {
          data = existingData.concat(data);
        }

        res.queryData.collectionData[pubName] = data;
      });
    }
    next();
  };
