...
http.OutgoingMessage.prototype.write = function(chunk, encoding) {
  ...
  if(this.queryData && !this.injected &&
    ...
    if(injectDataTemplate) {
      var data = EncodeEJSON(this.queryData.collectionData);
      var subscriptions = EncodeEJSON(this.queryData.subscriptions);
      var injectHtml = injectDataTemplate({data: data, subscriptions: subscriptions});
