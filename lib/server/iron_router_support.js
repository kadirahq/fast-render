  ...
  Router.routes.forEach(function(route) {
    ...
    if(route.options.fastRender) {
      handleRoute(route);
        ...
        route.findControllerConstructor && route.findControllerConstructor().prototype.fastRender ||
        route.findController && route.findController().prototype.fastRender /* for IR 0.9 */
    ) {
      handleRoute(route);
    }
  });
...
function handleRoute(route) {
  ...
  ['waitOn', 'subscriptions'].forEach(function(funcName) {
    var handler = ( route.options[funcName] ||
                    route.findControllerConstructor && route.findControllerConstructor().prototype[funcName] ||
                    route.findController && route.findController().prototype[funcName] /* for IR 0.9 */ );
  ...
  function onRoute(params, path) {
    ...
    var controller = route.options.controller;
