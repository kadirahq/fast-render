...
suite('Context', function() {
  ...
  test('subscribe:used-cursor', function(done, server, client) {
    var contextData = server.evalSync(function() {
      Posts = new Meteor.Collection('posts');
      Posts.insert({_id: '1', text: 'one', type: 'a'});
      Posts.insert({_id: '2', text: 'two', type: 'a'});
      ...
      Meteor.publish('posts', function(type) {
        var cursor = Posts.find({type: type}, {sort: {_id: -1}});
        cursor.fetch();
        return cursor;
      });
  ...
  test('subscribe:cursors-array', function(done, server, client) {
    var contextData = server.evalSync(function() {
      Posts = new Meteor.Collection('posts');
      Posts.insert({_id: '1', text: 'one', type: 'a'});
      Posts.insert({_id: '2', text: 'two', type: 'a'});
